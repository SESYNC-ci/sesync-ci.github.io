...
The configuration file (which stores host name, user name, and password) is created for each team by the cyberinfrastructure staff.  This keeps the information you don’t want to share private, and means you don’t have to constantly enter host URLs, user names, and passwords.  It will look like this: 

```{r config file, eval=FALSE}
[service_name]
...
NOTE: To rerun the dbGetQuery and reassign the result, you must first clear the existing result using the following code: `dbClearResult(q1)`
  
  
When finished, disconnect from the database.

```{r RStudio4, eval=FALSE}
...
```{python Jupyter1, eval=FALSE, python.reticulate = FALSE}
...
```{python Jupyter3, eval=FALSE, python.reticulate = FALSE}
# open database connection 
...
```{python Jupyter4, eval=FALSE, python.reticulate = FALSE}
# list the tables in the database
cur = mydb2.cursor()
...
```{python Jupyter5, eval=FALSE, python.reticulate = FALSE}
# disconnect from the database
mydb2.close()
...
If you would like to use your local installation of RStudio or Python on your local computer, you need to establish a secure connection via SSH Tunneling.  
...
Now your computer is securely connected to the SESYNC server and you can access the database from RStudio or Python installed on your local computer.  
...
You will need to copy the  ~/.pg_service.conf configuration file to the home directory on the gateway server.  
...
Then type in your password when prompted. The psql prompt will appear and look like `"%DATABASE_NAME%=>"`.
